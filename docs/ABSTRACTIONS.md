...
Adjudication is judgment over the subject's bounded report, not a re-derivation of the underlying work. A collaboration that wants deep review declares it as a work stage whose report the checkpoint adjudicates — criteria such as "the review identifies no blocking issues" or "the drafts converged" are then answerable by a small model or a human reading one report. Direct adjudication of a producer's own work remains expressible by making that work the subject.

For the active slice, one work task has at most one checkpoint, and one checkpoint may contain several criteria that must all be met. Checkpoints do not have checkpoints. There is no parallel checkpoint, review, or run entity: a reviewer is an ordinary work stage, and the checkpoint is where its report is judged.
...
A budgeted loop is part of the episode's collaboration policy, not part of the checkpoint decision. It declares an ordered path of one or more work stages, each with its own exact agent assignment and declared description, plus the checkpoint criteria and adjudicator, the maximum number of path attempts, and the success continuation. From outside, the loop reads as a single task: downstream work depends on the loop itself rather than on any materialized attempt or checkpoint, because any permitted attempt may be the one whose checkpoint records `met`. Attempts and their checkpoints are the loop's internal, immutable, materialized structure.

Materializing an attempt creates every stage as an immutable work task, chains consecutive stages with ordinary dependency edges, and creates the attempt's one checkpoint with the final stage as its subject. The loop body is exactly the part that repeats: work that should not repeat, such as planning before an implement-and-review loop, is declared outside the loop and connected by ordinary edges. There is no re-entry index, and the adjudicator never chooses where feedback re-enters.

The maximum counts the initial attempt. If `max_attempts` is three, the policy can materialize the path and its checkpoint at most three times. Infrastructure relaunches and same-session recovery resume one task and do not consume this budget.

A task's report is its result, and the loop policy passes reports mechanically. Within an attempt, a stage's prompt is assembled at launch from its immutable declared description plus the report of each prerequisite stage, and the assembled prompt is persisted in the task's artifacts, so a stage's bounded inputs stay inspectable without a transcript. Mush does not validate report content: the expected report shape belongs in the declared stage description, and a malformed report is a criteria failure for the checkpoint, not a parse failure.

When a checkpoint records `not_met` and budget remains, the policy materializes the path again, linking each stage to its counterpart through `previous_task_id`. The new first stage's description carries the declared stage description, the prior attempt's final-stage report, and the checkpoint's decision and evidence, so no agent authors another agent's instructions. The new checkpoint uses the same criteria and adjudicator. When an assignment is executable, the policy also queues what it materializes, so already-declared iteration advances without the declaring conversation; a human may still edit an unstarted materialized stage before it begins. The previous work results and checkpoint decisions remain unchanged.

`met` satisfies the loop's success continuation regardless of which permitted attempt produced it. `blocked` stops immediately. `not_met` with no remaining attempt stops as budget exhausted. Changing the stages, criteria, adjudicator policy, or maximum after execution begins is fresh judgment rather than another iteration of the declared loop.

## Evidence

Evidence begins as bounded Markdown stored with a task. A completed work task's result is its report — the bounded interface later stages, adjudicators, and humans consume, while durable outputs such as code travel through the worktree and repository. Work evidence can cite commands, tests, manually supplied references, observations, and unresolved concerns. Checkpoint evidence maps the decision back to the declared criteria instead of offering unbounded general commentary. Large harness execution output lives in task-owned artifact files whose paths stay in the task evidence; richer evidence types remain deferred.
...
Before a newly materialized attempt begins, its first stage's description carries the restart packet described in [the semantic-attempt budget](#semantic-attempt-budget), and a stage may reuse its counterpart's durable worktree when the declared policy permits that execution choice. A loop stage with prerequisite stages receives their reports in its launch prompt, which is persisted in its artifacts. Once execution begins, a task's purpose, relationships, and worktree choice remain immutable.
...
An edge's dependent is a work task with at most eight direct prerequisites. A work prerequisite is satisfied by successful completion, and a checkpoint prerequisite by a `met` decision. A loop's success continuation is part of the loop declaration rather than an edge permanently attached to any materialized checkpoint: the continuation task is gated on the loop itself without an explicit dependency, exactly as a checkpoint is gated on its subject, and `met` on any permitted attempt satisfies it while `not_met` and `blocked` advance nothing. An edge whose prerequisite is a loop member is refused, because depending on the loop's internal structure is the mis-declared continuation the loop record exists to replace; the loop's own stage-chaining edges are materialized by the policy. A checkpoint is additionally gated by its subject without any explicit dependency: it cannot become ready, execute, or receive a decision until its named work result exists. Self, duplicate, cross-project, cyclic, and post-start edge changes are rejected, and subject links participate in cycle prevention as readiness gates.
