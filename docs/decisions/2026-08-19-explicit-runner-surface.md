...
Status: accepted. Proposed on 2026-08-19 as a revision of the M4 dependency-chain slice; the implementation was accepted on 2026-08-20 after its focused invariant tests, the enforced merge gate, and an unattended three-node chain demonstration through `mush runner serve` all passed. Supersedes the launch-pump and same-boot liveness portions of [the M4 dynamic task graph decision](2026-08-18-m4-dynamic-task-graph.md). Its runner mechanisms remain implemented, while [the episodic execution graph decision](2026-08-20-episodic-execution-graphs.md) supersedes an incrementally extended live graph as the maintained product direction.
