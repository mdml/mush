...
pub(super) fn record_transition_and_advance(
    ...
    // A terminal transition can unblock edge dependents, the checkpoint that
    // reviews a completed work subject, and — when the task is a loop member
    // checkpoint recording `met` — the loop's declared success continuation.
    let dependents = connection.prepare("SELECT dependent_task_id FROM task_dependencies WHERE prerequisite_task_id=?1 UNION SELECT id FROM tasks WHERE subject_task_id=?1 AND kind='checkpoint' UNION SELECT continuation_task_id FROM loops WHERE continuation_task_id IS NOT NULL AND id=(SELECT loop_id FROM tasks WHERE id=?1) ORDER BY 1")?.query_map([task_id], |r| r.get::<_,i64>(0))?.collect::<Result<Vec<_>,_>>()?;
...
/// blocked while the checkpoint itself completes. A task declared as a loop's
/// success continuation is additionally gated on the loop itself, without an
/// explicit dependency, until any member checkpoint records `met`.
const PREREQUISITES_SATISFIED_SQL: &str = "NOT EXISTS(SELECT 1 FROM task_dependencies d JOIN tasks p ON p.id=d.prerequisite_task_id WHERE d.dependent_task_id=?1 AND (p.status!='completed' OR (p.kind='checkpoint' AND COALESCE(p.decision,'')!='met'))) AND NOT EXISTS(SELECT 1 FROM tasks c JOIN tasks s ON s.id=c.subject_task_id WHERE c.id=?1 AND s.status!='completed') AND NOT EXISTS(SELECT 1 FROM loops l WHERE l.continuation_task_id=?1 AND NOT EXISTS(SELECT 1 FROM tasks m WHERE m.loop_id=l.id AND m.kind='checkpoint' AND m.decision='met'))";
...
/// Give an unqueued pending task its durable queue readiness: bump the queue
/// generation, derive blocked or ready from the graph, create the launch
/// delivery when ready, and journal the transition. Callers validate first.
pub(super) fn queue_pending_task(connection: &Connection, task_id: i64) -> Result<(), DomainError> {
    let ready = prerequisites_satisfied(connection, task_id)?;
    let state = if ready { "ready" } else { "blocked" };
    connection.execute(
        "UPDATE tasks SET queue_generation=queue_generation+1,readiness_status=?2,intervention=NULL WHERE id=?1",
        params![task_id, state],
    )?;
    let generation = queue_generation(connection, task_id)?;
    if ready {
        connection.execute("INSERT OR IGNORE INTO launch_deliveries(task_id,generation,state) VALUES(?1,?2,'pending')", params![task_id,generation])?;
    }
    insert_transition(connection, task_id, "queued")?;
    ensure_launch_invariant(connection, task_id)
}
